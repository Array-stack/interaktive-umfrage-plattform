...
const db = new sqlite3.Database(DBSOURCE, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
    if (err) {
        ...
        db.run('PRAGMA foreign_keys = ON;', (err) => {
            if (err)
                console.error('Fehler beim Aktivieren der Fremdschlüssel:', err.message);
        });
        ...
        db.serialize(() => {
            ...
            db.run(`CREATE TABLE IF NOT EXISTS surveys (
                ...
                if (err) {
                    console.error("Fehler beim Erstellen der Tabelle 'surveys':", err.message);
                }
            });
            ...
            db.run(`CREATE TABLE IF NOT EXISTS questions (
                ...
                if (err) {
                    console.error("Fehler beim Erstellen der Tabelle 'questions':", err.message);
                }
            });
            ...
            db.run(`CREATE TABLE IF NOT EXISTS choices (
                ...
                if (err) {
                    console.error("Fehler beim Erstellen der Tabelle 'choices':", err.message);
                }
            });
            ...
            db.run(`CREATE TABLE IF NOT EXISTS responses (
                ...
                db.all("PRAGMA table_info(responses)", [], (err, columns) => {
                    ...
                    // Überprüfen, ob die ip_address-Spalte bereits existiert
                    const hasIpAddress = columns.some(column => column.name === 'ip_address' && column.type === 'TEXT');
                    if (hasIpAddress) {
            ...
            db.run(`CREATE TABLE IF NOT EXISTS answers (
                ...
                if (err) {
                    console.error("Fehler beim Erstellen der Tabelle 'answers':", err.message);
                }
            });
            ...
            db.run(`CREATE TABLE IF NOT EXISTS password_reset_tokens (
                ...
                if (err) {
                    console.error("Fehler beim Erstellen der Tabelle 'password_reset_tokens':", err.message);
                }
            });
        });
    }
