...
router.post('/surveys/:surveyId', authenticateToken, asyncHandler(async (req, res) => {
    try {
        ...
        const existingResponse = await new Promise((resolve, reject) => {
            db.get('SELECT * FROM responses WHERE survey_id = ? AND respondentId = ?', [surveyId, userId], (err, row) => err ? reject(err) : resolve(row));
        });
        if (existingResponse) {
            ...
            db.serialize(async () => {
                ...
                try {
                    ...
                    await new Promise((res, rej) => {
                        const stmt = db.prepare('INSERT INTO responses (id, survey_id, respondentId, submittedAt) VALUES (?, ?, ?, ?)');
                    ...
                    for (const [index, response] of responses.entries()) {
                        ...
                        try {
                            ...
                            /** @type {Promise<void>} */
                            await new Promise((res, rej) => {
                                const stmt = db.prepare('INSERT INTO answers (response_id, question_id, value) VALUES (?, ?, ?)');
                    ...
                    if (req.user && req.user.role === 'student') {
                        try {
                            ...
                            const surveyOwner = await new Promise((resolve, reject) => {
                                db.get('SELECT ownerId FROM surveys WHERE id = ?', [surveyId], (err, row) => err ? reject(err) : resolve(row?.ownerId));
                            });
                            if (surveyOwner) {
                                // Überprüfe, ob der Besitzer ein Lehrer ist
                                const isTeacher = await new Promise((resolve, reject) => {
                                    db.get('SELECT role FROM users WHERE id = ?', [surveyOwner], (err, row) => err ? reject(err) : resolve(row?.role === 'teacher'));
                                });
                                if (isTeacher) {
                                    // Überprüfe, ob die Beziehung bereits existiert
                                    const existingRelation = await new Promise((resolve, reject) => {
                                        db.get('SELECT id FROM teacher_students WHERE teacher_id = ? AND student_id = ?', [surveyOwner, userId], (err, row) => err ? reject(err) : resolve(row));
                                    });
                                    ...
                                    if (!existingRelation) {
                                        ...
                                        await new Promise((resolve, reject) => {
                                            db.run('INSERT INTO teacher_students (id, teacher_id, student_id, created_at) VALUES (?, ?, ?, ?)', [relationId, surveyOwner, userId, now], err => err ? reject(err) : resolve(undefined));
                                        });
...
router.get('/surveys/:surveyId', authenticateToken, asyncHandler(async (req, res) => {
    try {
        ...
        const responses = await new Promise((resolve, reject) => {
            db.all(`SELECT r.id, r.respondentId, r.submittedAt, u.username, u.email, u.role
         FROM responses r
         LEFT JOIN users u ON r.respondentId = u.id
         WHERE r.survey_id = ?
         ORDER BY r.submittedAt DESC`, [surveyId], (err, rows) => err ? reject(err) : resolve(rows));
        });
        ...
        const detailedResponses = await Promise.all(responses.map(async (response) => {
            const answers = await new Promise((resolve, reject) => {
                db.all(`SELECT a.question_id, a.value, q.text as question_text, q.type as question_type
           FROM answers a
           JOIN questions q ON a.question_id = q.id
           WHERE a.response_id = ?
           ORDER BY q.position`, [response.id], (err, rows) => err ? reject(err) : resolve(rows));
            });
...
router.get('/surveys/:surveyId/check', authenticateToken, asyncHandler(async (req, res) => {
    try {
        ...
        // Überprüfen, ob der Benutzer bereits an der Umfrage teilgenommen hat
        const existingResponse = await new Promise((resolve, reject) => {
            db.get('SELECT id, submittedAt FROM responses WHERE survey_id = ? AND respondentId = ?', [surveyId, userId], (err, row) => err ? reject(err) : resolve(row));
        });
        res.json({
