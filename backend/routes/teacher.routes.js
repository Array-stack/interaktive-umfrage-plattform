...
router.get('/students', authenticateToken, requireTeacherRole, asyncHandler(async (req, res) => {
  ...
  const students = await new Promise((resolve, reject) => {
    db.all(
      `SELECT ts.id, ts.added_at, ts.survey_id, 
...
router.post('/students/add-by-survey', authenticateToken, requireTeacherRole, asyncHandler(async (req, res) => {
  ...
  const survey = await new Promise((resolve, reject) => {
    db.get('SELECT * FROM surveys WHERE id = ? AND ownerId = ?', 
      [surveyId, teacherId], 
  ...
  const participants = await new Promise((resolve, reject) => {
    db.all(
      `SELECT DISTINCT r.respondentId, u.id, u.name, u.email, u.role
       FROM responses r
       JOIN users u ON r.respondentId = u.id
       WHERE r.survey_id = ? AND u.role = 'student'`,
      [surveyId],
  ...
  const existingRelations = await new Promise((resolve, reject) => {
    db.all(
      ...
      [teacherId],
      (err, rows) => err ? reject(err) : resolve(rows.map(r => r.student_id) || [])
    );
  });
  ...
  try {
    for (const student of participants) {
      ...
      if (existingRelations.includes(student.id)) continue;
      
      const relationId = generateId();
      await new Promise((resolve, reject) => {
        db.run(
          'INSERT INTO teacher_students (id, teacher_id, student_id, added_at, survey_id) VALUES (?, ?, ?, ?, ?)',
          [relationId, teacherId, student.id, now, surveyId],
          function(err) {
            if (err) return reject(err);
            addedStudents.push({
              id: relationId,
