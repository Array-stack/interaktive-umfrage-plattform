...
const db = new sqlite3.Database(DBSOURCE, (err) => {
    ...
    db.get("SELECT name FROM pragma_table_info('surveys') WHERE name = 'access_type';", (err, row) => {
        if (err) {
        ...
        if (!row) {
            ...
            db.run("ALTER TABLE surveys ADD COLUMN access_type TEXT DEFAULT 'public';", (err) => {
                if (err) {
                    ...
                    db.run("UPDATE surveys SET access_type = 'public' WHERE access_type IS NULL;", (err) => {
                        if (err) {
