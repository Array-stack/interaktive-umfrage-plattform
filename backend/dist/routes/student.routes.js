...
router.get('/surveys', authenticateToken, async (req, res) => {
    try {
        ...
        const publicSurveys = await new Promise((resolve, reject) => {
            db.all(`SELECT s.id, s.title, s.description, s.ownerId, s.createdAt, s.isPublic, s.access_type,
             ...
             ORDER BY isFromTeacher DESC, s.createdAt DESC`, [studentId], (err, rows) => {
                if (err) {
        ...
        const surveysWithStatus = await Promise.all(publicSurveys.map(async (/** @type {Survey} */ survey) => {
            try {
                // Verwende die Client-IP oder eine Standard-IP, falls nicht verfügbar
                const clientIp = req.ip || '127.0.0.1';
                const response = await new Promise((resolve, reject) => {
                    db.get(`SELECT COUNT(DISTINCT a.question_id) as answeredQuestions 
               FROM responses r
               JOIN answers a ON r.id = a.response_id
               WHERE r.survey_id = ? AND (r.respondentId = ? OR r.ip_address = ?)`, [survey.id, req.user?.userId || 'dev-user-id', clientIp], (err, row) => {
                        if (err) {
