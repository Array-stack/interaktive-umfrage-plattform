...
const { hashPassword, comparePassword, generateToken, generateAuthToken } = require('../authUtils');
const { generateEmailToken, sendVerificationEmail, sendPasswordResetEmail } = require('../emailUtils');
/**
...
router.get('/verify-email', async (req, res) => {
    ...
    try {
        db.get('SELECT * FROM users WHERE verification_token = ?', [token], async (err, user) => {
            ...
            db.run('UPDATE users SET email_verified = 1, verification_token = NULL, verification_token_expires = NULL WHERE id = ?', [user.id], function (err) {
                if (err) {
...
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        db.get('SELECT * FROM users WHERE email = ?', [email], async (err, user) => {
            ...
            db.run(sql, [tokenId, user.id, resetToken, expiresAt.toISOString(), 0, new Date().toISOString()], async function (err) {
                if (err) {
...
router.post('/reset-password', async (req, res) => {
    ...
    try {
        db.get('SELECT * FROM password_reset_tokens WHERE token = ? AND used = 0 AND expires_at > ?', [token, new Date().toISOString()], async (err, resetToken) => {
            if (err) {
                console.error('Datenbankfehler bei der Passwort-Zurücksetzung:', err.message);
                return res.status(500).json({ error: 'Serverfehler bei der Passwort-Zurücksetzung.' });
            }
            ...
            db.run('UPDATE users SET password = ? WHERE id = ?', [hashedPassword, resetToken.user_id], function (err) {
                ...
                db.run('UPDATE password_reset_tokens SET used = 1 WHERE id = ?', [resetToken.id], function (err) {
                    if (err) {
...
router.get('/profile', (req, res) => {
    ...
    db.get('SELECT id, email, name, role, created_at as createdAt, last_login as lastLogin FROM users WHERE id = ?', [userId], (err, user) => {
        if (err) {
